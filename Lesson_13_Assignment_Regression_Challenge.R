...
mvs$TrueStory <- factor(mvs$TrueStory)
mvs$ChickFlick <- factor(mvs$ChickFlick)
mvs$Action <- factor(mvs$Action)
mvs$TVPremiere <- factor(mvs$TVPremiere)
mvs$NetworkPremiere <- factor(mvs$NetworkPremiere)
mvs$ReRun <- factor(mvs$ReRun)
...
mod1 <- lm(Rating ~ Stars + Day + Hour + TrueStory + ChickFlick + Action + TVPremiere +
             NetworkPremiere + ReRun + PrevRating + SpotRatings + DomesticGross, data = mvs)
summary(mod1)
...
# removed.
mod2 <- lm(Rating ~ Stars + Day + Hour + TrueStory + ChickFlick + Action + TVPremiere +
             ReRun + PrevRating + SpotRatings + DomesticGross, data = mvs)
summary(mod2)
...
# are stored in *modelname$residuals*.) Save the plot as residplot.
residplot <- ggplot(mvs, aes(x = DomesticGross, y = mod2$residuals)) +
  geom_point()
...
#31. Create the new variable (DG2) needed to fix this problem.
mvs$DG2 <- mvs$DomesticGross^2

#32. Run your new model (as mod4), incorporating this new variable, and output a 
# summary of the model.
mod4 <- lm(Rating ~ Stars + Day + Hour + TrueStory + ChickFlick + Action + TVPremiere +
             ReRun + PrevRating + SpotRatings + DomesticGross + DG2, data = mvs)
summary(mod4)
...
mod5 <- lm(Rating ~ Stars + Day + Hour + TrueStory + ChickFlick + Action + TVPremiere +
             NetworkPremiere + ReRun + PrevRating + SpotRatings + DomesticGross + DG2, data = mvs)
summary(mod5)
...
mod6 <- lm(Rating ~ Stars + Day + Hour + TrueStory + ChickFlick + Action + TVPremiere +
             NetworkPremiere + ReRun + PrevRating + SpotRatings + DomesticGross + DG2 +
             PrevRating:SpotRatings, data = mvs)
summary(mod6)
...
moviesel$Stars <- factor(moviesel$Stars)
moviesel$TrueStory <- factor(moviesel$TrueStory)
moviesel$ChickFlick <- factor(moviesel$ChickFlick)
moviesel$Action <- factor(moviesel$Action)
moviesel$TVPremiere <- factor(moviesel$TVPremiere)
moviesel$NetworkPremiere <- factor(moviesel$NetworkPremiere)

#39. Next, you need to create the same variable we created in our previous model to 
# account for the nonlinear relationship.
moviesel$DG2 <- moviesel$DomesticGross^2
...
moviesel$Day <- rep("Friday", 20)
moviesel$Day <- factor(moviesel$Day)

#41. Add the *Hour* variable, factored, to the new data frame. Make them all 8.
moviesel$Hour <- rep("8", 20)
moviesel$Hour <- factor(moviesel$Hour)

#42. Add the *ReRun* variable, factored, to the new data frame. Make them all 0.
moviesel$ReRun <- rep("0", 20)
moviesel$ReRun <- factor(moviesel$ReRun)
...
moviesel$PrevRating <- rep(mean(mvs$PrevRating), 20)
...
moviesel$SpotRatings <- rep(mean(mvs$SpotRatings), 20)
...
predprofs <- predrats * 2400000 - moviesel$License.Fee
...
MovieSelect$MovieID[1] <- 15
MovieSelect$Day[1] <- "Sunday"
MovieSelect$Hour[1] <- 8
MovieSelect$SpotRatings[1] <- 50
MovieSelect$MovieID[2] <- 15
MovieSelect$Day[2] <- "Sunday"
MovieSelect$Hour[2] <- 12
MovieSelect$SpotRatings[2] <- 0
MovieSelect$MovieID[3] <- 15
MovieSelect$Day[3] <- "Saturday"
MovieSelect$Hour[3] <- 12
MovieSelect$SpotRatings[3] <- 0
